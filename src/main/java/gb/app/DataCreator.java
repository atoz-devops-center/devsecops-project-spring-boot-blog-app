package src.main.java.gb.app;

import gb.app.config.AppProperties;
import gb.app.config.Constants;
import gb.app.domain.AppUser;
import gb.app.domain.Authority;
import gb.app.domain.LiteUser;
import gb.app.domain.Note;
import gb.app.modules.note.NoteService;
import gb.app.modules.user.AuthorityService;
import gb.app.modules.user.UserService;
...
public class DataCreator {
    ...
    public void initData(){
        ...
        String pwd = "$2a$10$UtqWHf0BfCr41Nsy89gj4OCiL36EbTZ8g4o/IvFN2LArruHruiRXO"; //System.getenv("DATA_CREATOR_SECRET"); // to make it faster //value is 'pass'

        AppUser adminUser = new AppUser("system", "System", "Balimidi", "system@email");
        adminUser.setPassword(pwd);
        adminUser.setAuthorities(authorityService.findByNameIn(Constants.ROLE_ADMIN, Constants.ROLE_USER));
        userService.save(adminUser);

        AppUser user1 = new AppUser("user1", "Ganesh", "Balimidi", "balimidi.iibs@email");
        user1.setPassword(pwd);
        user1.setAuthorities(authorityService.findByNameIn(Constants.ROLE_USER));
        userService.save(user1);


        AppUser user2 = new AppUser("user2", "Raji", "Ganesh", "balimidi24@email");
